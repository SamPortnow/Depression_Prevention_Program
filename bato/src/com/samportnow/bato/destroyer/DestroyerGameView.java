...
public class DestroyerGameView extends View
{
	...
	protected void onDraw(Canvas canvas)
	{
		...
		if (mDrawPos)
		{
			...
			{
				canvas.drawBitmap(mDestroyer.mStationPositive.get(i - 1).getDrawingCache(), mDrawStationX + mMoveStationX, mDrawStationY, null);
		...
		if (mMovePos)
		{
			for (int i = 0; i < mPositive.size(); i++)
			{
				...
				if (mPosX < 0)
				{
					// if it gets to 0, populate the listview and stop drawing
		...
		if (mMoveNeg)
		{
			// moving the negative thought. it moves back and forth on the
			// screen
		...
		if (mDestroy)
		{
			...
			if (mMoveX >= mNegX - mDestroyer.mNeg.width / 2 && mMoveX <= mNegX + mDestroyer.mNeg.width && mMoveY > 0 && mMoveY < mDestroyer.mNeg.height / 2)
			{
